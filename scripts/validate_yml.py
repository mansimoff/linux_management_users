...
def main():
    ...
    files_to_check = sys.argv[1:] if len(sys.argv) > 1 else DEFAULT_FILES

    print(f"Проверка yaml синтаксиса ({len(files_to_check)} файлов)...")
    print()
    ...
    print()
    if errors:
        print(f"❌ Ошибок: {len(errors)} из {len(files_to_check)} файлов")
        sys.exit(1)
    else:
        print(f"✓ OK — все {len(files_to_check)} файлов корректны")
        sys.exit(0)
